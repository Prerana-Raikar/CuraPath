...
const PortalLayout: React.FC<PortalLayoutProps> = ({ navigation, children, title }) => {
            ...
            {navigation.map((item) => (
              <div
                ...
                className={`group flex items-center px-3 py-3 text-sm font-medium rounded-xl w-full text-left cursor-pointer transition-all duration-200 ${
                  location.pathname === item.href || (item.href !== '/patient' && item.href !== '/staff' && item.href !== '/doctor' && location.pathname.startsWith(item.href))
